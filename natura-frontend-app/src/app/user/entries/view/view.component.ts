...
export class ViewComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.dataSource.sortingDataAccessor = (item, property) => {
      switch (property) {
        case 'species.commonName':
          return item.species && item.species.commonName
            ? item.species.commonName.toLowerCase()
            : '';
        case 'proposer':
          return item.suggestedBy && item.suggestedBy.username
            ? item.suggestedBy.username.toLowerCase()
            : '';
        case 'date':
          return item.suggestedDate;
        case 'validated':
          return item.validatedBy !== null && item.validatedBy !== undefined
            ? 'true'
            : 'false';
        case 'validator':
          return item.validatedBy && item.validatedBy.username
            ? item.validatedBy.username.toLowerCase()
            : '';
        default:
      ...
      this.insectService.getById(id).subscribe(
        ...
        (error) => {
          this.loadingService.error(
            'Failed to load pollinating insect details !'
          );
        }
      );
    } else {
  ...
  getIdentifications() {
    this.identificationService.getByEntry(this.entry).subscribe(
      (data) => {
        ...
        if (this.isValidated) {
          const validated = this.identifications.find(
            (i) => i && i.validatedBy
          );
          this.species =
            validated && validated.species ? validated.species.commonName : '';
        }
  ...
  validate(identification, event) {
    if (this.isValidated) {
      this.loadingService.openErrorAlert(
        'This item already has a validated identification.'
      );
    } else {
      ...
      if (index === -1 || !validator || !species) {
        this.loadingService.openErrorAlert(
          'An error occurred. Validation cannot be processed.'
        );
      } else {
        if (
          this.canValidate &&
          validator.id !== identification.suggestedBy.id
        ) {
          ...
          dialogRef.afterClosed().subscribe((result) => {
            ...
            if (result) {
              ...
              this.identificationService
                .validate(identification, validator)
                .subscribe(
                  (data) => {
                  ...
                  (error) => {
                    ...
                    if (error.status === 406) msg = error.error.message;
                    this.loadingService.error(msg);
                  }
                );
            }
          ...
          this.loadingService.openErrorAlert(
            'You have not the permission to validate this identification.'
          );
        }
      }
    }
  ...
  viewLikes(identification: Identification): void {
    if (
      identification &&
      identification.likes &&
      identification.likes.length > 0
    ) {
